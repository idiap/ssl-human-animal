...
def get_model(upstream_model, device="cuda"):
    return upstream_model.eval().to(device)
...
def get_features_original(model, padded_wavs, device, return_attentions):
    with torch.no_grad():
        ...
        out = model(padded_wavs, output_attentions=return_attentions)
        features = out
        if return_attentions:
            attentions = out.attentions
            attentions = torch.stack(attentions, dim=1)
...
def extract(cfg: DictConfig):
    ...
    model: LightningModule = hydra.utils.instantiate(cfg.upstream_model)
